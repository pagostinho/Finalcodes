...
if __name__ == "__main__":
	if len(sys.argv) != 5:
		...
		img_path = glob.glob("/home/irocs/Desktop/IROCS/Data/Supervised/trainLiquids/*.png")
		gt_path = glob.glob("/home/irocs/Desktop/IROCS/Data/Supervised/gtLiquids/*.png")
		...
		if img_path == 0:
			...
			for i in range(dim):
				...
				network = per.Perceptron(Vec_train[i],Vec_gt)
				network.train()
				for j in range(len(X_test[0][0])):
					X_result[i].append(network.test(X_test[i][0][j]))
			...
			print("Accuracy = " + str(float(count)/float(len(X_result[0]))*100))
